...
void setSphereData(double x, double y, double z, double sphereRadius)
{
  ...
  for (phi1 = 0; phi1 < phiMax; phi1 += phiDelta)
  {
    for (theta1 = 0; theta1 < thetaMax; theta1 += thetaDelta)
    {
      ...
      pts[nPts].x = x + r * costh1 * sinph1;    
      pts[nPts].y = y + r * sinth1 * sinph1;    
      pts[nPts].z = z + r * cosph1;    
      nPts++;

      pts[nPts].x = x + r * costh2 * sinph1;    
      pts[nPts].y = y + r * sinth2 * sinph1;    
      pts[nPts].z = z + r * cosph1;    
      nPts++;

      pts[nPts].x = x + r * costh2 * sinph2;    
      pts[nPts].y = y + r * sinth2 * sinph2;    
      pts[nPts].z = z + r * cosph2;    
      nPts++;
      
      pts[nPts].x = x + r * costh1 * sinph2;    
      pts[nPts].y = y + r * sinth1 * sinph2;    
      pts[nPts].z = z + r * cosph2;    
      nPts++;
